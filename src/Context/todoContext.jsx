...
export const TodoProvider = ({ children }) => {
  ...
  const getTodos = async () => {
    let res = await fetch("http://localhost:8000/api/todo/get-todo", {
      method: "GET",
      headers: {
  ...
  const updateImportant = async (id, important)=> {
    try {
      let res = await fetch(`http://localhost:8000/api/todo/update-important-status/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("token"),
  ...
  const addTodo = async (title, checked) => {
    let res = await fetch("http://localhost:8000/api/todo/add-todo", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": localStorage.getItem("token"),
  ...
  const updateTodo = async (id, title) => {
    let res = await fetch(`http://localhost:8000/api/todo/update-todo/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "auth-token": localStorage.getItem("token"),
  ...
  const updateCheck = async (id,checked)=>{
    try {
      let res = await fetch(`http://localhost:8000/api/todo/update-checked-status/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("token"),
  ...
  const deleteTodo = async (id) => {
    let res = await fetch(`http://localhost:8000/api/todo/delete-todo/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "auth-token": localStorage.getItem("token"),
